  ...
  return (
    <div className="carousel carousel-center space-x-4 p-4 w-full">
      {tokens.map((token: Token) => (
        <div key={token.symbol} className="carousel-item transition delay-150 duration-300 ease-in-out hover:-translate-y-1 hover:scale-102" onClick={() => navigate(`/tokens/${token.symbol}${token.id ? `?id=${token.id}` : ''}`)}>  
